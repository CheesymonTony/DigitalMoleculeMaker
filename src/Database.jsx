...
const getMoleculeProperties = (selectedImages) => {
  ...
  const lightAbsorptionSum = molecules.reduce((acc, currImage) => {
    return acc + currImage.reactivity;
  }, 0);
  const lifespanSum = molecules.reduce((acc, currImage) => {
    return acc + currImage.activation_energy;
  }, 0);
  const bandGapSum = molecules.reduce((acc, currImage) => {
    return acc + currImage.bandgap;
  }, 0);

  return [
    weightSum / numMolecules,
    lightAbsorptionSum / numMolecules,
    lifespanSum / numMolecules,
    bandGapSum / numMolecules,
  ];
};
